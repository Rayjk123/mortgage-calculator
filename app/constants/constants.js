export const FORM_UPDATE = 'FORM_UPDATE';

export const MORTGAGE_AMOUNT = 'Mortgage Amount';
export const MORTGAGE_INTEREST = 'Mortgage Interest';
export const MORTGAGE_YEARS = 'Mortgage Years';

export const MORTGAGE_TITLE = 'Mortgage Calculator';
export const MORTGAGE_RESULTS_TITLE = 'Your Results Summary';
export const MORTGAGE_RESULTS_DETAILS = 'View Detailed Results';
export const MORTGAGE_TOTAL_COST = 'Total';
export const MORTGAGE_MONTHLY_PAY = 'Monthly';

export const Results = {
	RESULTS_MONTHLY_PRINCIPAL: 'Monthly Principal',
	RESULTS_NUMBER_OF_MONTHLY_PAYMENTS: 'Number of Payments',
	RESULTS_TOTAL_PAYMENTS: 'Total Principal',
	RESULTS_ORIGINAL_LOAN_AMOUNT: 'Original Loan Amount'
};

export const MAX_TOTAL_LENGTH = 10;
export const LOL = '~~~~~~~~~~~~~~~~~~~~~~~~~~';
